...
f <- filtered %>% 
  ...
  dplyr::summarise(first = min(EVENT_TIME),
            last = max(EVENT_TIME))
...
head(phy_census)
# unique(filtered$EVENT_NAME)
filtered_outlwbs <- filtered %>%
  filter(PAT_ENC_CSN_ID != p_id)

p_id <- unique(filtered$PAT_ENC_CSN_ID[filtered$EVENT_NAME == 'LWBS Disposition Selected'])

patient <- filtered_outlwbs %>% 
  filter(EVENT_NAME == 'Assign Attending') %>% 
  mutate(date = date(EVENT_TIME),
         hour = hour(EVENT_TIME)) %>% 
  group_by(date, hour) %>% 
  dplyr::summarise(patient_taken = n())

patient1 <- filtered_outlwbs %>%
  filter(EVENT_NAME == 'Patient departed from ED') %>%
  mutate(date = date(EVENT_TIME),
         hour = hour(EVENT_TIME)) %>% 
  group_by(date, hour) %>%
  dplyr::summarise(patient_out = n())

patient2 <- filtered_outlwbs %>%
  filter(EVENT_NAME == 'Patient discharged') %>%
  mutate(date = date(EVENT_TIME),
         hour = hour(EVENT_TIME)) %>% 
  group_by(date, hour) %>%
  dplyr::summarise(patient_out = n())

patient3 <- filtered_outlwbs %>%
  filter(EVENT_NAME == 'Pt transferred to Checkout') %>%
  mutate(date = date(EVENT_TIME),
         hour = hour(EVENT_TIME)) %>% 
  group_by(date, hour) %>%
  dplyr::summarise(patient_out = n())

write_csv(patient3, 'report/data/patient3.csv')


patient$patient_out  <- patient1$patient_out
  

summary <- total_final %>% 
  select(date, hour, numBeds) %>% 
  inner_join(phy_census, by = c('date','hour')) %>% 
  inner_join(patient2, by = c('date','hour')) %>%
  select(-time) %>%
  dplyr::rename(physician = count)
...
table(hmm$hour[hmm$utilization>1])
output <- hmm %>% 
  mutate(year = year(date)) %>%
  filter(hour %in% c(18, 19, 20, 21, 22,23, 0), year == 2017, utilization > 0.8) %>% 
  group_by(physician, hour) %>% 
  dplyr::summarise(avg = mean(patient_out), numObservations = n(), flow=max(avg/physician)) %>%
  filter(numObservations > 10)
mean(output_18$flow)

## average rate of patient/physician = 3.52
hmm_18 <- hmm %>%
  filter(hour %in% c(18, 19, 20))

output_18 <- hmm_18 %>% 
  mutate(year = year(date)) %>%
  filter(hour %in% c(18, 19, 20, 21, 22,23, 0), year == 2017, utilization > 0.8) %>% 
  group_by(physician, hour) %>% 
  dplyr::summarise(avg = mean(patient_taken), numObservations = n(), flow=max(avg/physician)) %>%
  filter(numObservations > 10)
